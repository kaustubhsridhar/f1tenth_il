...
def bc(seed, agent, expert, env, start_pose, observation_shape, downsampling_method, render, render_mode, purpose, vgain_scales):
    ...
    for vgain_scale in vgain_scales:
        resume_poses[vgain_scale] = start_pose
    ...
    for iter in range(n_iter + 1):
        ...
        # Collect data from the many experts
        print("Collecting data from the expert...")
        traj = {"observs": [], "poses_x": [], "poses_y": [], "poses_theta": [], "scans": [], "actions": [], "reward": 0}
        
        for vgain_scale in vgain_scales:
            # Expert control policy params
            tlad = 0.82461887897713965
            vgain = 0.90338203837889 * vgain_scale

            # starting data collection
            done = False
            obs, step_reward, done, info = env.reset(resume_poses[vgain_scale])
            ...
            if is_last_round_done:
                ...
                resume_poses[vgain_scale] = np.array([[obs["poses_x"][0], obs["poses_y"][0], obs["poses_theta"][0]]])
